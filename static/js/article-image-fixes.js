(()=>{
  ...
  const exactRules=[
    [/scheffler-qi4d-core-switch/,OFFICIAL.qi4dCore],
    [/scheffler-old-wedges/,OFFICIAL.vokeySm8],
    [/ventus-headspeed-40/,OFFICIAL.ventusTr],
    [/spider-tour-2026/,OFFICIAL.spiderTour]
  ];
  ...
  function exactImage(path){
    ...
    const hit=exactRules.find(([re])=>re.test(p));
    return hit?hit[1]:'';
  }
  function topicImage(text,path){
    const s=(String(text||'')+' '+String(path||'')).toLowerCase();
    if(/リシャフト|シャフト|shaft/.test(s))return topicImages.reshaf​​t||topicImages.resh​​aft;
    ...
    if(/コース|ラウンド|course/.test(s))return topicImages.course;
    if(/スイング|練習|アプローチ|バンカー|swing/.test(s))return topicImages.swing;
    if(/初心者|beginner/.test(s))return topicImages.beginner;
    return '';
  }
  ...
  document.querySelectorAll('.archive-card').forEach((card,index)=>{
    ...
    let src=exactImage(path)||topicImage(title,path);
    if(!src)return;
    src+=(src.includes('?')?'&':'?')+'jirothumb='+(index+1);
    ...
    thumb.dataset.imageSource=exactImage(path)?'manufacturer-official':'topic-related';
  });

  const hero=document.querySelector('.cat-hero');
  if(hero){
    const src=exactImage(location.pathname);
    if(src){hero.style.backgroundImage=`linear-gradient(180deg,rgba(5,10,6,.08),rgba(5,10,6,.86)),url('${src}')`;hero.dataset.articleImageFixed='1';hero.dataset.imageSource='manufacturer-official';}
  }
})();